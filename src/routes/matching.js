...
// ─────────────────────────────────────────────────────────────────────────────
// GET /api/matching/passed-by-me
// Profiles I passed (left-swiped) — for swipe history "Passed" tab
// ─────────────────────────────────────────────────────────────────────────────
router.get("/passed-by-me", async (req, res) => {
  try {
    const me = await User.findById(req.user._id).select("passedUsers");
    if (!me.passedUsers?.length) return res.json({ success: true, users: [] });

    const users = await User.find({
      _id: { $in: me.passedUsers },
      onboardingComplete: true,
    }).select("name age bio photos kundli gender");

    return res.json({ success: true, users });
  } catch (err) {
    console.error("[MATCHING] passed-by-me error:", err);
    return res.status(500).json({ success: false, message: err.message });
  }
});
...
// Body: { reason: 'fake' | 'spam' | 'offensive' | 'underage' | 'other' }
//
// For now: stores a simple report doc in MongoDB.
// Future: add admin dashboard or email alert.
// ─────────────────────────────────────────────────────────────────────────────

// ── You'll need a Report model — add this to src/models/Report.js ─────────────
/*
 const mongoose = require('mongoose');
 const ReportSchema = new mongoose.Schema({
   reporter: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
   reported: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
   reason:   { type: String, enum: ['fake', 'spam', 'offensive', 'underage', 'other'], required: true },
...
router.post("/report/:userId", async (req, res) => {
  try {
    ...
    // Require Report model — add it to models/Report.js first (schema above)
    const Report = require("../models/Report");
    ...
    if (existing) {
      return res
        .status(200)
        .json({ success: true, message: "Report already submitted" });
    }
    ...
    // Also add to passedUsers so they don't appear in Discover again
    await User.findByIdAndUpdate(req.user._id, {
      $addToSet: { passedUsers: reportedId },
    });

    console.log(
      `[REPORT] ${req.user._id} reported ${reportedId} for: ${reason}`
    );
    return res.status(200).json({ success: true, message: "Report submitted" });
  } catch (err) {
    console.error("[MATCHING] report error:", err);
    return res.status(500).json({ success: false, message: err.message });
  }
});
