  ...
  io.on("connection", (socket) => {
    ...
    socket.on("message:send", async ({ matchId, text, tempId }, cb) => {
      try {
        ...
        const receiver = await User.findById(otherId).select(
          "blocked pushToken"
        );

        if (
          sender.blocked?.map((id) => id.toString()).includes(otherId) ||
          receiver.blocked?.map((id) => id.toString()).includes(socket.userId)
        ) {
          return cb?.({ error: "Cannot send message" });
        }

        const moderation = moderateMessage(text);
        console.log("[SOCKET] moderation result:", moderation, "text:", text);
        if (!moderation.allowed) {
          return cb?.({ error: moderation.reason });
        }
        ...
        if (!onlineUsers.has(otherId)) {
          if (
            receiver?.pushToken?.startsWith("ExponentPushToken") &&
            receiver.pushToken !== sender.pushToken
          ) {
            fetch("https://exp.host/--/api/v2/push/send", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                to: receiver.pushToken, // was other.pushToken
